...
![License](https://img.shields.io/badge/license-Apache--2.0-blue) ![Gemini 3.5 Flash](https://img.shields.io/badge/Gemini-3.5%20Flash-4285F4) ![Google ADK](https://img.shields.io/badge/Google-ADK-34A853)
...
<img src="docs/architecture.png" width="840" alt="Warden reference architecture">
...
<img src="docs/factory.png" width="760" alt="The factory pipeline">
...
```mermaid
flowchart TD
    CRON[Monthly trigger - no human in the loop] --> COLLECT[Evidence files arrive]
    COLLECT --> PARSE["Deterministic parsers<br/>(CSV grammar; assumed timezone carries provenance)"]
    COLLECT --> PROSE[Prose summary]
    PROSE --> READER["Reader: Gemini proposes a POINTER<br/>(the figure + its verbatim sentence)"]
    READER --> LEASH{"Re-grounding leash:<br/>quote verbatim? figure stated?"}
    LEASH -->|no| REFUSE1[Extraction refusal, recorded]
    LEASH -->|yes| BATCH[Evidence batch]
    PARSE --> BATCH
    BATCH --> GATES["Layer 1 - structural gates<br/>(deterministic, file-level, no thresholds)"]
    GATES -->|coverage broken| REFUSED[REFUSED - no number emitted]
    GATES --> HOLDS["Layer 2 - semantic holds<br/>(refined by evidence, row-level)"]
    HOLDS -->|"empty activity trail"| EXCL["EXCLUDED - decided against,<br/>evidence cited, no one asked"]
    HOLDS -->|"dense activity trail"| FLOOR
    HOLDS -->|"absent / ambiguous signal"| HELD["HELD - a person asked,<br/>decidable_with named"]
    GATES --> FLOOR["Layer 3 - the attested floor<br/>(every digit traces to a row)"]
    FLOOR --> REPORT["Reporter: floor + pending + rejected + excluded,<br/>quoted whole by the agent"]
```
...
```mermaid
flowchart LR
  CS[Cloud Scheduler] --> API[HackOps on Cloud Run]
  GH[GitHub] --> API
  API --> ADK[Google ADK]
  ADK --> G[Gemini 3.5 Flash / Vertex AI]
  ADK --> T[Grounded Audit Tool]
  API --> FS[(Firestore)]
  T --> A[Submission Artifacts]
  FS --> UI[Proof-of-Action Timeline]
```
...
```mermaid
flowchart LR
  subgraph untrusted["customer-authored text"]
    mail[inbound email]
    notes[(account notes<br/>customer-writable)]
  end

  subgraph fleet["Fleet"]
    armor[Model Armor]
    triage["<b>triage</b><br/>gemini-3.7-flash · LOW<br/>reads customer text<br/><b>no write credential</b>"]
    disp["<b>dispatcher</b><br/>not a model<br/>deterministic, explains itself"]
    subgraph general["fleet · service account fleet-server"]
      an["<b>analyst</b><br/>read-only + storage"]
      sc["<b>scribe</b><br/>drafting only"]
    end
    subgraph alone["fleet-operator · its own service account, no public URL"]
      op["<b>operator</b><br/>4 named back-office actions<br/>the only worker that can write"]
    end
    review["review queue<br/><i>people</i>"]
    calib[("calibration<br/>approvals · redos<br/>last corrections")]
    ledger[("audit_log<br/>append-only")]
  end

  acme[("AcmeCorp Admin<br/>separate service<br/>per-action grants")]
  ui["board · queue · workforce<br/>onSnapshot, read-only"]

  mail --> armor --> triage --> disp
  disp -->|"tasks lane: worker != operator"| general
  disp -->|"tasks lane: worker = operator"| alone
  disp -->|tier 2, or under the bar| review
  op --> acme
  an --> acme
  notes -.customer text arrives here too.-> an
  general --> review
  alone --> review
  review -->|approve| calib
  review -->|send back + instructions| calib
  calib -->|rate| disp
  calib -->|corrections replayed into the prompt| general
  disp --> ledger
  general --> ledger
  alone --> ledger
  review --> ledger
...
![SwarmOps system architecture — deterministic governance separated from the AI layer](docs/architecture/system-architecture.png)
...
```mermaid
flowchart TB
  subgraph Console["Fleet Command UI"]
    OP[Procurement operator]
  end
  subgraph Gateway["Agent Gateway — Cloud Run"]
    ID[Agent Identity JWT]
    POL[Policy / residency]
    ARM[Model Armor]
    REG[Agent Registry]
  end
  subgraph Runtime["Agent Runtime — ADK + Gemini 3.5"]
    ORCH[Orchestrator]
    IN[Intake Partner]
    CO[Compliance]
    SE[Security]
    PR[Procurement]
    AX[Action Executor]
    LG[Logistics Coordinator]
    MEM[Memory Curator]
  end
  subgraph State["Firestore Memory Bank + systems of record"]
    MB[(Memories / sessions)]
    ERP[(ERP inventory)]
    VND[(Vendor master / tickets / lanes)]
  end
  PS[Pub/Sub async jobs]
  WH[Carrier delivery webhook]
  OP --> Gateway
  Gateway --> Runtime
  ORCH --> IN & CO & SE & PR & AX & LG & MEM
  Runtime --> State
  Runtime --> PS
  WH --> Gateway
  ARM --> ID
...
```markdown
<p align="center">
  <img src="docs/logo_horizontal.png" alt="MRP Sentinel" width="800">
</p>
...
```mermaid
flowchart LR
    subgraph UNTRUSTED["UNTRUSTED — vendor-authored"]
        DOC["documents/<br/>invoice image + memo text<br/>(may carry injected instructions)"]
    end

    subgraph THINK["THINK ZONE — reads anything, can do nothing"]
        INV["VARIANCE INVESTIGATOR ×10<br/>Gemini via ADK · <b>zero tools</b><br/>owns no collection"]
        SCHEMA{"VarianceFinding schema<br/>frozen · extra=forbid<br/><b>id must match ^[a-z0-9-]+$</b>"}
    end

    subgraph TIE["TIE-OUT LANE — recompute, never trust"]
        TIEOUT["TIE-OUT CHECKERS ×7 · no LLM<br/>P1 row-level AP↔GL · P4 register<br/>P5 chain verify · P8 flux scan"]
    end

    subgraph ACT["ACT ZONE — no LLM anywhere"]
        MATCH{"opaque id → reviewed<br/>je_templates library<br/>applies_to · preconditions"}
        AMT{"amount-source allowlist<br/>tie_out_delta · register_recompute<br/>intake_stated_total · matched_pair"}
        POST["POSTING ENGINE ×1<br/>balanced · capped · period-gated<br/>post_once — one atomic transaction"]
        LEDGER[("postings/<br/>hash-chained, append-only<br/>CREATE only — nobody UPDATEs")]
    end

    HUMAN["CONTROLLER<br/>escalation queue · period lock<br/>(the only path to LOCKED)"]

    DOC -->|"full text + image bytes"| INV
    INV -->|"proposes"| SCHEMA
    SCHEMA -->|"✗ malformed id, URL, path"| HUMAN
    SCHEMA -->|"✓ suggested_template_id ONLY<br/>— the sole field that crosses"| MATCH
    MATCH -->|"✗ not in library / wrong lane"| HUMAN
    MATCH --> AMT
    TIEOUT -->|"probe receipts — arithmetic<br/>over the books, stamped on the case"| AMT
    AMT -->|"✗ no receipt · over cap · sign-flip"| HUMAN
    AMT --> POST
    POST --> LEDGER
    POST -->|"postcondition re-tie fails ×2<br/>kill rule · lane quarantined"| HUMAN
    LEDGER -.->|"P5 verify cadence<br/>→ postings_meta/verify_status"| TIEOUT
    TIEOUT -->|"opens variance_cases/"| INV
    HUMAN -.-> POST

    style UNTRUSTED fill:#fde8e8,stroke:#c0392b,color:#111
    style THINK fill:#e8f0fb,stroke:#2f6fba,color:#111
    style TIE fill:#f1edf9,stroke:#6f4fbf,color:#111
    style ACT fill:#e6f6ec,stroke:#2e8b57,color:#111
    style HUMAN fill:#fff4dc,stroke:#c9952a,color:#111
```
...
![Gatehouse architecture](docs/architecture.png)

Source of the diagram (Mermaid):

```mermaid
flowchart LR
    U[Untrusted email / attachment] --> N[Naked control agent]
    N --> MT[Mock tools]

    subgraph TB["Untrusted reading room · no tool handles"]
      direction LR
      U --> Q["LLM · ADK LlmAgent<br/>Gemini 3.5 Flash<br/>tools = []"]
      Q --> QR["QuarantineReport<br/>trust = untrusted"]
      QR --> S["Rules + LLM judge<br/>Gemini structured output"]
    end

    S --> P["Pure code policy<br/>SanitizedBrief"]
    P --> D{Pure code decision}
    D -->|deny| B[Blocked]
    D -->|require approval| H[Pure code human gate]
    D -->|allow| E[Typed executor]
    H --> E
    E --> MT
    Q -. structured event .-> A[Hash-chain audit JSONL]
    S -. structured event .-> A
    P -. structured event .-> A
    H -. structured event .-> A
    E -. structured event .-> A
```
